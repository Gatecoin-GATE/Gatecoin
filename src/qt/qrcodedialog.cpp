...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("GATE")
    updateDisplayUnit();
}
...
{
    QString ret = QString("gatecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GATE unit input in lnReqAmount, we generate the URI with GATE as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::GATE, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
